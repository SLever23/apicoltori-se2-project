...
=======
beforeAll(function () {
    //db.submissions.push({id: 0, user: 0, exam: 0, task: 0, response: 'prova'}, {id: 1, user: 0, exam: 0, task: 1, response: 'prova'});
})
...
describe('Test Get all submissions', () => {
    ...
    test('Get all with invalid references', () => {
        expect(submission_logic.sumbission_get_all(123, 0, 0)).toEqual([]);
        expect(submission_logic.sumbission_get_all(0, 0, 74)).toEqual([]);
        expect(submission_logic.sumbission_get_all(0, -23, 0)).toEqual([]);
    })

    //Not Error
    test('Get all valid', () => {
        expect(db.submissions.length).toBe(2);
        expect(submission_logic.sumbission_get_all(0, 0)).toEqual([{id: 0, user: 0, exam: 0, task: 0, response: 'prova'}, {id: 1, user: 0, exam: 0, task: 1, response: 'prova'}]);
